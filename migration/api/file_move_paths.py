...
def catch_all(path):
    ...
    if request.method == 'PUT':
        ...
        for i in body:
            try:
                client.data[collection].insert_one(validate_type(i, move_type))
                response.append("OK")
            except:
                response.append("ERR")
        ...
        try:
            return Response(json.dumps([{key:str(value) if type(value) is ObjectId else value for key, value in item.items()} for item in list(client.data[collection].find(validate_query(body, move_type)))]), mimetype='application/json')
        except:
        ...
        data = client.data[collection].delete_many(validate_query(body, move_type))
