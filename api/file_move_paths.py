...
def catch_all(path):
    ...
    if request.method == 'PUT':
        ...
        body = request.json
        #try:
        return Response(json.dumps([{key:str(value) for key, value in item.items()} for item in list(client.data.file_move_paths.find(validate_query(body, move_type)))]), mimetype='application/json')
        #except:
